...
4. **メタデータフィルタリング**セクションでメタデータのフィルタリング条件を設定し、メタデータ機能を使用してナレッジベース内の文書をフィルタリングします
5. **機能追加**内で**引用と帰属**をオンにする
6. **デバッグとプレビュー**内でナレッジベース関連のユーザー質問を入力してデバッグ
7. デバッグが完了したら、**保存して公開**し、AIナレッジベースQ\&Aアプリケーションとして公開
...
2. **手動モード**を選択した場合は、以下の手順でフィルタリング条件を設定してください：
    ...
    <tr>
        ...
        <td>is</td>
        <td>フィールドの値は入力した値と完全に一致する必要があります。例えば、フィルタリング条件を is <code>"Published"</code> に設定した場合、「Published」とマークされた文書のみが返されます。</td>
    </tr>
    <tr>
        <td>is not</td>
        <td>フィールドの値は入力した値と一致してはいけません。例えば、フィルタリング条件を <code>is not "Draft"</code> に設定した場合、「Draft」とマークされていないすべての文書が返されます。</td>
    </tr>
    <tr>
        <td>is empty</td>
    ...
    <tr>
        <td>contains</td>
        <td>フィールドの値に入力したテキストが含まれています。例えば、フィルタリング条件を <code>contains "Report"</code> に設定した場合、「Monthly Report」や「Annual Report」など、「Report」を含むすべての文書が返されます。</td>
    </tr>
    <tr>
        <td>not contains</td>
        <td>フィールドの値に入力したテキストが含まれていません。例えば、フィルタリング条件を <code>not contains "Draft"</code> に設定した場合、「Draft」を含まないすべての文書が返されます。</td>
    </tr>
    <tr>
        <td>starts with</td>
    ...
    <tr>
        <td>ends with</td>
        <td>フィールドの値が入力したテキストで終わります。例えば、フィルタリング条件を <code>ends with "2024"</code> に設定した場合、「Report 2024」や「Summary 2024」など、「2024」で終わるすべての文書が返されます。</td>
    </tr>
    <tr>
    ...
    <tr>
        <td>before</td>
        <td>フィールドの時間値は選択した時間より前でなければなりません。例えば、<code>before "2024-01-0"</code> は2024年1月1日より前とマークされているすべての文書を返します。</td>
    </tr>
    <tr>
        <td>after</td>
